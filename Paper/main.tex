...
\title{\boldmath Isocurvature-induced features in multi-field Higgs-$R^2$ inflation }%Primordial features and low-$\ell$ suppression from isocurvature modes in multi-field Higgs–$R^2$ inflation}
...
We study primordial perturbations in Higgs-$R^2$ inflation in the presence of a non-minimal kinetic mixing between the Higgs and the scalaron. By performing a full numerical integration of the multifield background and linear perturbations, we identify distinct dynamical regimes controlled by the non-minimal coupling $\xi_h$. For $\xi_h \sim \mathcal{O}(0.1)$, the turning rate of the inflationary trajectory induces transient multifield effects that imprint localized features in the scalar power spectrum. In the limit $\xi_h \ll 1$, the adiabatic spectrum becomes featureless, but isocurvature modes do not fully decay, leaving a residual contribution at the end of inflation. We compute the corresponding CMB angular power spectra and illustrate how these regimes lead to characteristic imprints across different multipole ranges. Our results show that suppressing spectral features does not necessarily eliminate isocurvature perturbations, placing non-trivial constraints on multifield Higgs-$R^2$ realizations and motivating future probes sensitive to primordial isocurvature.}
...
We now turn to the observational signatures of the Higgs-$R^2$ model in the CMB angular power spectra. Figure~\ref{fig7} (top panel) displays the temperature (TT) power spectrum for the benchmark parameter $\xi_h = 0.1$, compared with the $\Lambda$CDM baseline, and current CMB measurements from Planck~\cite{planck2018x} and ACT DR6~\cite{louis2025} shown for reference. The lower panel shows the fractional deviations with respect to $\Lambda$CDM, illustrating the scale dependence of the multifield effects. We can observe remarkable features related to the angular spectrum at larges and small scales for the case $\xi_h = 0.1$. 

At large angular scales ($\ell \lesssim 30$), the coupling between adiabatic and isocurvature perturbations induced by the turning trajectory leads to a suppression of power in the Sachs--Wolfe plateau~\cite{Jain2009}. This suppression overlaps with the multipole range probed by current temperature measurements, but should be interpreted as a qualitative imprint of the inflationary dynamics rather than as a quantitative explanation of any specific anomaly, given the large cosmic variance affecting these scales. At intermediate and small angular scales ($\ell \gtrsim 500$), the same mechanism produces a coherent suppression of power across the TT, TE, and EE spectra (see Fig~\ref{fig8}). While the $\Lambda$CDM baseline and the $\xi_h = 10^{-9}$ case remain close to the reference measurements, the $\xi_h = 0.1$ scenario exhibits a systematic reduction in power. This behavior reflects the scale-dependent redistribution of primordial curvature perturbations induced by transient multifield dynamics and highlights an intrinsic limitation of suppressing isocurvature perturbations in this class of models.



\begin{figure}[t]
    \centering
    \includegraphics[width = 1 \textwidth]{Figures/TT_angular_spectrum.pdf}

    \caption{Comparison of the TT angular power spectrum for two representative choices of the Higgs non-minimal coupling $\xi_h$. For $\xi_h = 10^{-9}$ the resulting spectrum closely follows the $\Lambda$CDM baseline, while for $\xi_h = 0.1$ transient multifield dynamics induce a scale-dependent suppression at low multipoles, $\ell \lesssim 40$.}
    \label{fig7}
\end{figure}
